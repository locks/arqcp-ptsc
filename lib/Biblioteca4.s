...
;io_printString ( addr  -- )
;desc:
;		imprime no terminal uma string colocada na memória. 
...
;
;desc:
;		lê da consola um byte para a stack
;
;input:
;
;output:
...
;
;desc:
;		lê do "terminal" uma string terminada por CR 
;	tendo em atenção o tamanho da string. 
;		A função deve escrever na memória os caracteres
;	lidos a partir de um determinado endereço, e guardar
;	no fim da string o byte 0 (zero), deixando na stack
;	o número de caracteres lidos. 
;		O  "Carriage Return" não deve ser guardado na
;	memória nem deve ser contado o backspace que elimina
;	os caracteres já digitados.
;
;input:
;		addr: endereço do strings (bytes)
...
;
;desc:
;		a função recebe um valor(positivo) de 32 bits e
;	imprime-o no terminal.
;
;input:
;		n	: valor a imprimir
;output:
...
;
;desc:
;		a função lê da consola um valor(positivo) de
;	32 bits até o CR seja digitado.
;
;input:
;
;output:
